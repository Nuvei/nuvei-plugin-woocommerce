...
class Nuvei_Pfw_Notify_Url extends Nuvei_Pfw_Request {
	...
	public function process() {
		$logged_params = array(
			...
			'responseTimeStamp'         => Nuvei_Pfw_Http::get_param( 'responseTimeStamp', 'string' ),
			'advanceResponseChecksum'   => Nuvei_Pfw_Http::get_param( 'advanceResponseChecksum', 'string' ),
		);
...
// 		if ( $responsechecksum !== $checksum ) {
		if ( !hash_equals($responsechecksum, $checksum) ) {
			$log_data = array();
...
//	private function search_order_by_dmn_data( $trans_id, $transaction_type = '', $order_id = 0 ) {
	...
	private function create_auto_void( $transaction_type ) {
		...
		wp_send_json_success('Problem with Auto-Void request.');
	}
...
//	}
	...
	private function process_auth_sale_dmn( $transaction_type, $client_request_id, $transaction_id, $req_status ) {
		...
		$this->check_for_repeating_dmn($transaction_id, $req_status);
	...
	private function process_settle_void_dmn( $order_id, $req_status, $transaction_type, $client_unique_id ) {
		...
		$this->is_order_valid( $order_id );
		$this->check_for_repeating_dmn(Nuvei_Pfw_Http::get_param( 'TransactionID' ), $req_status);
